...
function minimax_value() {
    ...
    var parent_node_idx = gameTreeArray[node_idx + slot_node_id_parent] * node_size;

    if (gameTreeArray[node_idx + slot_number_childs] == 0) { //is a terminal node
        current_node_id = gameTreeArray[node_idx + slot_node_id_parent];  // sets the next current_node_id for the simulation
        returned_child_value = gameTreeArray[node_idx + slot_value]; 		// next returned_child_value: the heuristic value of node
        return;
    }

    if (gameTreeArray[node_idx + slot_current_child] == -1) { // passing alpha and beta values from parent node
        if (parent_node_idx >= 0) {
            gameTreeArray[node_idx + slot_alpha] = gameTreeArray[parent_node_idx + slot_alpha];
            gameTreeArray[node_idx + slot_beta] = gameTreeArray[parent_node_idx + slot_beta];
        }
        if (gameTreeArray[node_idx + slot_node_level] % 2 == 0)
            gameTreeArray[node_idx + slot_value] = MINUS_INFINITY; // max player initial value
        else
            gameTreeArray[node_idx + slot_value] = PLUS_INFINITY; // min player initial value
        gameTreeArray[node_idx + slot_current_child]++;
        current_node_id = gameTreeArray[node_idx + slot_id_first_child]; // sets the next current_node_id for the simulation
        return;
    }
    ...
    if (gameTreeArray[node_idx + slot_node_level] % 2 == 0) { // max player
        if (returned_child_value > gameTreeArray[node_idx + slot_value]) {
            ...
            gameTreeArray[node_idx + slot_best_child] = gameTreeArray[node_idx + slot_id_first_child] + gameTreeArray[node_idx + slot_current_child];
        }
        if (gameTreeArray[node_idx + slot_value] >= gameTreeArray[node_idx + slot_beta]) { // Alpha-beta pruning
        ...
        gameTreeArray[node_idx + slot_alpha] = Math.max(gameTreeArray[node_idx + slot_alpha], gameTreeArray[node_idx + slot_value]);
        gameTreeArray[node_idx + slot_current_child]++;
        if (gameTreeArray[node_idx + slot_current_child] < gameTreeArray[node_idx + slot_number_childs]) { // calls next child
            current_node_id = gameTreeArray[node_idx + slot_id_first_child] + gameTreeArray[node_idx + slot_current_child];
        ...
        if (gameTreeArray[node_idx + slot_value] <= gameTreeArray[node_idx + slot_alpha]) { // Alpha-beta pruning
            draw_prune_nodes(node_idx);
            current_node_id = gameTreeArray[node_idx + slot_node_id_parent];
            returned_child_value = gameTreeArray[node_idx + slot_value];
            return;
        }
        gameTreeArray[node_idx + slot_beta] = Math.min(gameTreeArray[node_idx + slot_beta], gameTreeArray[node_idx + slot_value]);
        gameTreeArray[node_idx + slot_current_child]++;
        if (gameTreeArray[node_idx + slot_current_child] < gameTreeArray[node_idx + slot_number_childs]) { // calls next child
            current_node_id = gameTreeArray[node_idx + slot_id_first_child] + gameTreeArray[node_idx + slot_current_child];
        } else { // no more childs: returns to parent
...
function draw_prune_nodes(parent_idx) {
    ...
    while (child_id < number_childs) {
        child_idx = (gameTreeArray[parent_idx + slot_id_first_child] + child_id) * node_size;
...
function enter_node(idx_node) {
    ...
    if (gameTreeArray[idx_node + slot_node_level] % 2 == 0)
        context.fillText(step_minimax, cx - 5, cy + 8); // prints step number for max player
    else
        context.fillText(step_minimax, cx - 5, cy - 3); // prints step number for min player
    ...
    if (gameTreeArray[idx_node + slot_number_childs] > 0) {
        ...
        // updated node info
        context.fillStyle = "black";
        context.fillText("\u237a=" + gameTreeArray[idx_node + slot_alpha], xx, cy - 10); // alpha value
        context.fillText("\u03b2=" + gameTreeArray[idx_node + slot_beta], xx, cy); // beta value
        context.fillText("v=" + gameTreeArray[idx_node + slot_value], xx, cy + 10); // node value
    }
}
...
function prepare_draw_tree(numLeafNodes) {
    ...
    for (var i = tree_number_nodes - numLeafNodes, j = 1, idx_node = i * node_size; i < tree_number_nodes; i++, j++, idx_node += node_size) {
...
function print_tree() {
    document.form1.MyMessagesArea.value = "tree_height=" + tree_height + ";tree_number_nodes=" + tree_number_nodes + ";Tree>";
    for (var i = 0, len = gameTreeArray.length; i < len; i++) {
        document.form1.MyMessagesArea.value = document.form1.MyMessagesArea.value + gameTreeArray[i] + ",";
    }
